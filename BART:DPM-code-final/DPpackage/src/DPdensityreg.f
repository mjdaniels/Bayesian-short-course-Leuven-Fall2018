c=======================================================================                      
      subroutine dpdenregr(nrec,nx,nvar,z,npred,
     &                     xpred,ngrid,grid,
     &                     a0b0,k0,nuvec,s2inv, 
     &                     s2invm2, 
     &                     psiinv2,tau,mcmc,nsave,cpo,
     &                     thetasave,randsave,densp0,denspm,denspl,
     &                     meanfpm,meanfpl,meanfph, 
     &                     meanfpm1,meanfpl1,meanfph1, 
     ...
     &                     workm1,workm2,workm3,workmh1,workmh2,ywork,
     &                     iflagx,workvx,workmx,worksam,numcpo,denomcpo)
c=======================================================================                      
c     # of arguments = 63.
c
...
c+++++Data
      integer nrec,nx,nvar
c+      integer missp(nmiss,2)  
      real*8 z(nrec,nvar)
...
c+++ change 1
      real*8 densp0(nsave,npred)
      real*8 denspm(nsave,ngrid)
      real*8 denspl(nsave,ngrid)
      real*8 meanfpm(npred,ngrid)
      real*8 meanfpl(npred,ngrid)
      real*8 meanfph(npred,ngrid)
      real*8 meanfpm1(npred,ngrid)
      real*8 meanfpl1(npred,ngrid)
      real*8 meanfph1(npred,ngrid)
...
c+++++RNG and distributions
      real*8 dnrm,rnorm,rgamma,cdfnorm
...
c+        if(nmissi.eq.1)then

c++++++++++ check if the user has requested an interrupt
c+            call rchkusr()
c+
c+            do ii=1,nrec
c+               z(ii,2)=zhat(iscan,ii)
c+            end do
c+         end if 
...
c+++++++++++++ random effects
               ...
               do i=1,nrec
                  ...
                  do j=1,nvar
                     counter=counter+1
                     randsave(isave,counter)=muwork(j)
                  end do   
                  
                  do j=1,nvar
                     do k=j,nvar
                        counter=counter+1
                        randsave(isave,counter)=sigmawork(j,k)
                     end do
                  end do               
               end do
...
c+++++++++++++ Partially sampling the DP and computing the cond. dist.
     ...
     &                       workmx(j,k)*(xpred(i,k)-workvx(k))
                  ...
                  do j=1,ngrid  
                num(i,j)=tmp1*dnrm(grid(j),muc,sigmac,0)*tmp7
                num1(i,j)=tmp1*cdfnorm(grid(j),muc,sigmac,0,0)*tmp7
                  end do 
                  denom(i)=tmp1*tmp7
               end do
               do l=1,nrec
               ...
               do while((1.d0-tmp2).gt.eps)
                  ...
                  if(evali.le.ncluster)then
c                     do i=1,nvar  
c                        muwork(i)=muclus(evali,i)
c                        do j=1,nvar
c                           sigmawork(i,j)=sigmaclus(evali,
c     &                                              ihmssf(i,j,nvar))
c                        end do
c                     end do  
                  wt(evali)=wt(evali)+tmp1
                  end if
...
c                  end if
     ...
     &                       workmx(j,k)*(xpred(i,k)-workvx(k))
                     ...
                     do j=1,ngrid  
                        num(i,j)=num(i,j)+
     &                  tmp1*dnrm(grid(j),muc,sigmac,0)*tmp7
                        num1(i,j)=num1(i,j)+
     &                  tmp1*cdfnorm(grid(j),muc,sigmac,0,0)*tmp7
     ...
     &                       workmx(j,k)*(xpred(i,k)-workvx(k))
                     ...
                     do j=1,ngrid  
                        num(i,j)=num(i,j)+
     &                  wt(evali)*dnrm(grid(j),muc,sigmac,0)*tmp7
                        num1(i,j)=num1(i,j)+
     &                  wt(evali)*cdfnorm(grid(j),muc,sigmac,0,0)*tmp7

                     end do 
                     denom(i)=denom(i)+wt(evali)*tmp7
                 end do
				  end if
			   end do
...
c++++++++++++++++ change here for DIC
c+               do j=1,npred
c+                  densp0(isave,j)=0.d0
c+               end do
c+++++++++++++++ end change


c+++++++++++++++++ Bayesian bootstrap
               call dirichlet(dalpha,npred,npred,diri)


               do i=1,npred
                  call rchkusr()
c+++++++++++++++++++ change here used to calculate DIC
c+                    densp0(isave,i)=num(i,i)/denom(i)
c++++++++++++++++++++ end change
                  do j=1,ngrid
                     fs(j)=num(i,j)/denom(i)
                     fs1(j)=num1(i,j)/denom(i)
                     denspm(isave,j)=denspm(isave,j)+diri(i)*fs(j)
                     denspl(isave,j)=denspl(isave,j)+diri(i)*fs1(j)
                     meanfpm(i,j)=meanfpm(i,j)+fs(j)
                     meanfpm1(i,j)=meanfpm1(i,j)+fs1(j)

                  end do
                  write(1) (fs(j),j=1,ngrid)
                  write(2) (fs1(j),j=1,ngrid)
               end do 
...
c         call rchkusr()
c         do j=1,ngrid
c            denspm(i,j)= denspm(i,j)/dble(npred)
c            denspl(i,j)= denspl(i,j)/dble(npred)
c         end do
c      end do

      do i=1,npred
         do j=1,ngrid
            meanfpm(i,j)= meanfpm(i,j)/dble(nsave)
            meanfpm1(i,j)= meanfpm1(i,j)/dble(nsave)
         end do
      end do
     ...
     &                   worksam,fs,meanfpl,meanfph)

         call hpddensreg1(nsave,npred,ngrid,0.05d0,tband,
     &                     worksam,fs1,meanfpl1,meanfph1)
      end if
...
c=======================================================================      
      subroutine hpddensreg1(nsave,npred,ngrid,alpha,tint,
